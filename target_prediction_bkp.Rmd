...
gutmgene_m <- read.csv("data/gutmgene/gutmgene_microbal_met.csv", stringsAsFactors = FALSE)
gutmgene_h <- read.csv("data/gutmgene/gutmgene_host_gene.csv", stringsAsFactors = FALSE)
...
gutmgene_m <- gutmgene_m[!grepl("Bacteria Latreille et al. 1825", gutmgene1$Gut_Microbe_Name), ]
# Remove not identified species
gutmgene_m <- gutmgene_m[!grepl("CEBAS", gutmgene1$Gut_Microbe_Name), ]

# Save data with microbe info
save(gutmgene_m, file = "data/gutmgene/gutmgenem_beforemerging.RData")
save(gutmgene_h, file = "data/gutmgene/gutmgeneh_beforemerging.RData")
...
gutmgene_bind$Metabolite_CID <- str_match(
  gutmgene_bind$Metabolite_ID,
  "\\((\\d+)(?:,\\s*CHEBI[^)]*)?\\)\\s*$"
)[, 2]

# cids <- map(unique_mets, get_cid, .progress = TRUE)
# cids <- get_cid_vec(unique_mets)

# Parse Compound ID (CID) with vectorized function

# gutmgene_bind <- gutmgene_bind %>%
#   mutate(
...
save(gutmgene_bind, file = "data/gutmgene/gutmgene_bind.RData")  # No duplicated metabolites
...
df_smiles_ids <- data.frame(SMILES=smiles,
                         pubchem_ids=paste0("cid", all_cid),
                         stringsAsFactors=FALSE)
# Save the dataframe as text file
write.table(df_smiles_ids, file="data/gutmgene/smiles_withcid.txt",
...
smiles_cids <- read.table("data/gutmgene/smiles_all.txt", sep = " ", header = FALSE)
...
# Lê o arquivo txt com separador de espaço
dados <- read.table("data/gutmgene/smiles_all.txt", sep = " ", header = FALSE)
# Pega só a segunda coluna como vetor
...
```{r}
# Read STP dataframe
multitask_result <- read.csv("data/target_prediction/multitask/multitask_results.csv")
...
write.csv(stp_mappings, "data/target_prediction/stp/stp_mappings.csv", row.names = FALSE)
write.csv(all_stp, "data/target_prediction/stp/stp_targets.csv", row.names = FALSE)

# Multitask CHEMBL
multitask_uniprot_ids <- read.table("data/target_prediction/multitask/multitask_uniprot_ids.txt",
                                     header = FALSE,
                                     stringsAsFactors = FALSE) 
...
# PPB2
ppb_uniprot_ids <- read.table("data/target_prediction/ppb2/ppb_uniprot_ids.txt",
                                     header = FALSE,
                                     stringsAsFactors = FALSE) 

ppb_chembl_ids <- read.table("data/target_prediction/ppb2/ppb_chembl_ids.txt", header = FALSE)

ppb_from_chembl_mapping <- chembl_database_mapping(ppb_chembl_ids$V1)
ppb_from_chembl_mapping$Source <- "chembl_database"
ppb_from_uniprot_mapping <- map_hgnc_all_methods(ppb_uniprot_ids$V1)
...
write.csv(ppb_mappings, "data/target_prediction/ppb2/ppb_mappings.csv", row.names = FALSE)
...
all_stp <- read_csv("data/target_prediction/stp/stp_targets.csv", show_col_types = FALSE)$x
all_ppb <- read_csv("data/target_prediction/ppb2/ppb_targets.csv", show_col_types = FALSE)$x
...
target_metabolites_stp <- get_targets_to_metabolites("data/target_prediction/stp")
save(target_metabolites_stp, file="data/target_prediction/stp/target_metabolites_stp.RData")
```
...
target_metabolites_multitask_chemblid <- get_targets_to_metabolites("data/target_prediction/multitask")
...
multitask_chembl_to_uniprot <- read.csv("data/target_prediction/multitask/multitask_chembl_uniprot.csv")
...
for (target in names(target_metabolites_multitask_chemblid)) {
  ...
  uniprot_targets <- multitask_chembl_to_uniprot$uniprot_id[
    multitask_chembl_to_uniprot$chembl_id == target
  ]
...
save(target_metabolites_multitask, file="data/target_prediction/multitask/target_metabolites_multitask.RData")
```
...
target_metabolites_ppb_chemblid <- get_targets_to_metabolites("data/target_prediction/ppb2")
names(target_metabolites_ppb_chemblid) <- trimws(names(target_metabolites_ppb_chemblid))
# save(target_metabolites_multitask_chemblid, file="data/target_prediction/multitask/target_metabolites_multitask_chemblid.RData")

ppb_chembl_to_uniprot <- read.csv("data/target_prediction/ppb2/ppb_chembl_uniprot.csv")
...
for (target in names(target_metabolites_ppb_chemblid)) {
  ...
  # Get all UniProt IDs for this target
  uniprot_targets <- ppb_chembl_to_uniprot$uniprot_id[
    ppb_chembl_to_uniprot$chembl_id == target
  ]
...
save(target_metabolites_ppb, file="data/target_prediction/ppb2/target_metabolites_ppb.RData")
```