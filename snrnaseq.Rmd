...
degs_GSE213982 <- run_DE_analysis(GSE213982, save = TRUE,
...
degs_GSE144136 <- run_DE_analysis(GSE144136, save = TRUE,
                                  output_path = paste0(gse_path, "degs/"),
                                  prefix = "/GSE144136")
GSE144136 <- NULL
gc()
...
threshold <- quantile(combined_GSE213982$condition, 0.85)
...
threshold <- quantile(combined_GSE144136$condition, 0.85)
...
ppcor_network_analysis <- function(expr_matrix, query_genes){
  ...
  E(g)$weight <- sig_results$Estimate
  ...
  nodes$cor_degree <- norm(nodes$degree)
  nodes$cor_bet <- norm(nodes$bet)
  nodes$cor_close <- norm(nodes$close)
  nodes$cor_eigen <- norm(nodes$eigen)
  ...
  plot_grafo <- function(metric_col, title) {
    ggraph(layout) +
        geom_edge_link(aes(width = log1p(abs(E(g)$weight))),
                       alpha = 0.3) +
  ...
  p1 <- plot_grafo(nodes$cor_degree, "Degree")
  p2 <- plot_grafo(nodes$cor_bet, "Betweenness")
  p3 <- plot_grafo(nodes$cor_close, "Closeness")
  p4 <- plot_grafo(nodes$cor_eigen, "Eigenvector")
...
```{r}
bhe_genes <- c("F3", "CLDN10", "GJA1", "APOE", "S1PR1", "EDNRB", 
               "GLUL", "ATP1B2", "FGFR3", "AGT", "DDIT4")

expr_matrix_GSE213982 <- get_expr_matrix(GSE213982, bhe_genes)
GSE213982 <- NULL
gc()
p_bhe_GSE213982 <- ppcor_network_analysis(expr_matrix_GSE213982, bhe_genes)

expr_matrix_GSE144136 <- get_expr_matrix(GSE144136, bhe_genes)
GSE144136 <- NULL
gc()
p_bhe_GSE144136 <- ppcor_network_analysis(expr_matrix_GSE144136, bhe_genes)
```
...
merge_integrated_chunks <- function(chunks_dir) {
  chunk_files <- list.files(chunks_dir, pattern = "\\.rds$", full.names = TRUE)
  if (length(chunk_files) == 0) stop("Nenhum arquivo .rds encontrado em: ", chunks_dir)
  
  message("Lendo e juntando chunks...")
  merged <- readRDS(chunk_files[1])
  
  for (i in 2:length(chunk_files)) {
    message("Merge chunk ", i, " de ", length(chunk_files))
    chunk <- readRDS(chunk_files[i])
    merged <- merge(merged, chunk)
  }
...
merged_obj <- merge_integrated_chunks(gse_path)
...
```{r}
degs_merged_expr <- read.csv(paste0(gse_path, "degs/merged_results_filtered.csv"))
degs_merged <- degs_merged_expr$X
## GSEA

merged_gene_fc <- degs_merged_expr$avg_log2FC
names(merged_gene_fc) = degs_merged_expr$X
merged_gene_fc = sort(merged_gene_fc, decreasing = TRUE)

merged_gsea_mf <- gseGO(geneList   = merged_gene_fc,
                      OrgDb        = org.Hs.eg.db,
                      ont          = "MF",
                      minGSSize    = 100,
                      maxGSSize    = 500,
...
edox <- setReadable(merged_gsea_mf, 'org.Hs.eg.db', 'ENTREZID')
p_merged_gsea_mf <- cnetplot(edox, categorySize="geneNum", foldChange=merged_gene_fc)

merged_gsea_bp <- gseGO(geneList      = merged_gene_fc,
                      OrgDb        = org.Hs.eg.db,
                      ont          = "BP",
                      minGSSize    = 100,
                      maxGSSize    = 500,
...
edox <- setReadable(merged_gene_fc, 'org.Hs.eg.db', 'ENTREZID')
p_merged_gsea_mf <- cnetplot(edox, categorySize="geneNum", foldChange=merged_gene_fc)
```