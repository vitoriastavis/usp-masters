...
rnaseq_genes <- c("FGFR3")

targets_to_metabolites_fgfr3 <- get_metabolites(rnaseq_genes)
# targets_to_metabolites_fgfr3 <- targets_to_metabolites_fgfr3$FGFR3
# targets_to_metabolites_fgfr3 <- ifelse(
#   grepl("^nocid", targets_to_metabolites_fgfr3),
#   targets_to_metabolites_fgfr3,
#   paste0("cid", targets_to_metabolites_fgfr3)
# )
# smiles_ids <- read.table("data/gutmgene/smiles_ids.txt")
# smiles_ids_fgfr3 <- smiles_ids[smiles_ids$V2 %in% targets_to_metabolites_fgfr3,]
# write.table(smiles_ids_fgfr3, file = "data/gutmgene/smiles_ids_fgfr3.txt", row.names = F, col.names = F, quote = F)

for (target in names(targets_to_metabolites_fgfr3)){
  mets <- targets_to_metabolites_fgfr3[[target]]
  new_mets <- c()
  for (met in mets){
    # cid -> name
  ...
  targets_to_metabolites_fgfr3[[target]] <- new_mets
}

metabolites_to_targets_fgfr3 <- list()
for (gene in names(targets_to_metabolites_fgfr3)) {
  # print(gene)
  for (metabolite in targets_to_metabolites_fgfr3[[gene]]) {
    # print(metabolite)
    metabolites_to_targets_fgfr3[[metabolite]] <- c(metabolites_to_targets_fgfr3[[metabolite]], gene)
  }
}
metabolites_to_targets_fgfr3 <- lapply(metabolites_to_targets_fgfr3, unique)

save(targets_to_metabolites_fgfr3, metabolites_to_targets_fgfr3, file="data/targets_metabolites_fgfr3.RData")
```
