...
create_graph <- function(enrichment, geneset, remove_terms){ 
  cores <- c(
    "#6FAF9C", "#A665B5", "#87CEE0", "#b6e8b3",
    "#8e83de", "#1b2c9e", "#ECCFFF", "#1E6951",
    "#C1CFD9", "#300145", "#ff2eee", "#00c947"
  )
      ...
      graph <- cnetplot(readable, showCategory = nrow(readable@result),
                        size_item = 4, color_item = "gray50", 
                        node_label = "item", size_edge = 2,
                        color_edge = 'gray20', layout = "dh"
                        )
...
# Return - none
save_files <- function(barplot, graph, geneset, path,
                       barplot_h = 18, barplot_w = 20,
...
# load the data 
all_targets <- read.table("../targets.txt")$V1
...
enrichments <- run_enrichment(gene_ids, p_value, minGSSize = 2)
```
...
remove_terms <- c("Muscle tissue development", "Striated Muscle tissue development", "Taxis",
                  "Muscle organ development", "Neuron to neuron synapse", "Postsynaptic specialization membrane", "Postsynaptic density membrane", "neuron spine", "Glutamate receptor signaling pathway", "Regulation of synaptic transmission, glutamatergic", "Regulation of postsynaptic membrane potential", "Negative regulation of synaptic transmission, glutamatergic", "Learning", "Excitatory postsynaptic potential", "Chemical synaptic transmission, postsynaptic", "Response to cocaine", "Ligand-gated ion channel signaling pathway", "Negative regulation of synaptic transmission", "Telencephalon development", "Regulation of neuronal synaptic plasticity", "Regulation of long-term neuronal synaptic plasticity", "Calcium-mediated signaling", "Ligand-gated monoatomic ion channel activity involved in regulation of presynaptic membrane potential", "Neurotransmitter receptor activity involved in regulation of postsynaptic
membrane potential", "Transmitter-gated monoatomic ion channel activity involved in regulation of postsynaptic membrane potential", "Ligand-gated monoatomic ion channel activity involved in regulation of presynaptic membrane potential", "Neurotransmitter receptor activity involved in regulation of postsynaptic membrane potential", "Glutamate-gated receptor activity", "Glutamate-gated calcium ion channel activity", "Glutamte receptor activity", "Transmitter-gated monoatomic ion channel activity", "Transmitter-gated channel activity", "Neurotransmitter receptor activity involved in regulation of postsynaptic cytosolic calcium ion concentration", "Extracellular ligand-gated monoatomic ion channel activity", "Carboxylic acid transmembrane transporter activity", "Amino acid transmembrane transporter activity")
...
for (geneset in names(enrichments)){
  ...
  # e@result$Description <- stringr::str_to_sentence(e@result$Description)
  
  # filter  categories
  categories <- filter_categories(e, p_value, n_categories, remove_terms)

  # print(categories)
  # categories <- categories[!tolower(categories) %in% tolower(remove_terms)]
  # print("barplot \n")
  # creating barplot
  bar_plot <- create_barplot(e,
                            geneset,
                            categories)
  ...
  # save_files(bar_plot, graph, geneset, output_path)
  #
  if (geneset %in% c('REACTOME')) {
    save_files(bar_plot, graph, geneset, output_path, 12, 16, graph_w = 14, graph_h = 16)
  } else if (geneset %in% c('KEGG')) {
    save_files(bar_plot, graph, geneset, output_path, 5, 16, graph_w = 10, graph_h = 12)
  } else if (geneset %in% c('DGN')) {
    save_files(bar_plot, graph, geneset, output_path, 10)
  } else if (geneset %in% c('MF')) {
    save_files(bar_plot, graph, geneset, output_path, 14, graph_h = 22, graph_w=20)
  } else{
    save_files(bar_plot, graph, geneset, output_path, 12, graph_h = 22,  graph_w=20)
  }
} 
```