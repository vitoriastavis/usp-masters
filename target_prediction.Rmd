...
sea_mapped <- map_to_hgnc(sea_targets)
stp_mapped <- map_to_hgnc(stp_targets)

write.csv(stp_mapped, "data/target_prediction/stpnovo/stp_targets.csv", row.names = FALSE)
write.csv(sea_mapped, "data/target_prediction/sea/sea_targets.csv", row.names = FALSE)

# Nova interseção
targets_intersection <- intersect(sea_mapped, stp_mapped)
length(targets_intersection)
...
teng_i <- read_excel("data/teng3.xlsx", col_names = TRUE)
