...
library(dplyr)
# Directory with STP .csv files
directory <- "data/target_prediction/chemblpred"
...
# Concatenate dataframes
chembl_results <- bind_rows(dataframes)

# Write final dataframe
write_csv(chembl_results, file.path(directory, "chemblpred_results.csv"))
```

```{r}
# Read STP dataframe
chembl_result <- read.csv("data/target_prediction/chemblpred/chemblpred_results.csv")

# Rename columns to avoid special character issues
colnames(chembl_result) <- c("Target_ID", "Probability")

# Remove targets with probability = 0
chembl_result <- chembl_result[chembl_result$Probability != 0, ]

# Filter the rows with higher probability
chembl_result <- chembl_result[order(chembl_result$Target_ID, chembl_result$Probability), ]

# Get unique targets
chembl_result <- chembl_result %>% distinct(Target_ID, .keep_all=TRUE)
chembl_targets <- chembl_result$Target_ID  

write.table(chembl_targets, file = "data/target_prediction/chemblpred/chembl_chembls.txt",col.names=FALSE, row.names = FALSE,quote = FALSE)

cat(length(chembl_targets), "targets were predicted with ChemblPred\n")
```
...
# Iterate over the files creating dataframes
dataframes_xfp <- list()
dataframes_ecfp <- list()
for (file_path in files) {
  df <- read_tsv(file_path, show_col_types = FALSE)
  if(dim(df)[1] > 0){
    
    if (grepl("Xfp", file_path)){
      dataframes_xfp[[length(dataframes_xfp) + 1]] <- as.data.frame(df)
    } else if (grepl("ECfp4", file_path)){
      dataframes_ecfp[[length(dataframes_ecfp) + 1]] <- as.data.frame(df)
    }
  } else {
    cat("No targets predicted:", basename(file_path), "\n")
  }
...
# Concatenate dataframes
xfp_results <- bind_rows(dataframes_xfp)
ecfp_results <- bind_rows(dataframes_ecfp)

# Write final dataframe
write_csv(xfp_results, file.path(directory, "xfp_results.csv"))
write_csv(ecfp_results, file.path(directory, "ecfp_results.csv"))
```

```{r}
library(dplyr)
# Read STP dataframe
xfp_result <- read.csv("data/target_prediction/ppb2/xfp_results.csv")
ecfp_result <- read.csv("data/target_prediction/ppb2/ecfp_results.csv")

# Rename columns to avoid special character issues
colnames(xfp_result) <- c("Rank", "Target_ID", "Common.name")
colnames(ecfp_result) <- c("Rank", "Target_ID", "Common.name")

# Get unique targets
xfp_result <- xfp_result %>% distinct(Target_ID, .keep_all=TRUE)
ecfp_result <- ecfp_result %>% distinct(Target_ID, .keep_all=TRUE)

xfp_targets <- xfp_result$Target_ID 
ecfp_targets <- ecfp_result$Target_ID  

write.table(xfp_targets, file = "data/target_prediction/ppb2/xfp_chembls.txt", col.names=FALSE, row.names = FALSE, quote = FALSE)
write.table(ecfp_targets, file = "data/target_prediction/ppb2/ecfp_chembls.txt",col.names=FALSE, row.names = FALSE,quote = FALSE)

cat(length(xfp_targets), "targets were predicted with PPB2 - Xfp\n",
    length(ecfp_targets), "targets were predicted with PPB2 - ECfp4\n")
```
...
chembl_uniprot_ids <- read.table("data/target_prediction/chemblpred/uniprot_ids_chembl.txt",
                                 header = FALSE,
                                 stringsAsFactors = FALSE) 
chembl_mappings <- map_hgnc_all_methods(chembl_targets)
chembl_mappings_python <- map_hgnc_all_methods(chembl_uniprot_ids$V1)
all_chembl <- unique(chembl_mappings$HGNC_ID)
all_chembl <- all_chembl[!is.na(all_chembl)]
write.csv(chembl_mappings, "data/target_prediction/chemblpred/chemblpred_mappings.csv", row.names = FALSE)
write.csv(all_chembl, "data/target_prediction/chemblpred/chemblpred_targets.csv", row.names = FALSE)
...
all_xfp <- unique(xfp_mappings$HGNC_ID)
all_xfp <- all_xfp[!is.na(all_xfp)]
write.csv(xfp_mappings, "data/target_prediction/ppb2/xfp_mappings.csv", row.names = FALSE)
write.csv(all_xfp, "data/target_prediction/ppb2/xfp_targets.csv", row.names = FALSE)

ecfp_mappings <- map_hgnc_all_methods(ecfp_targets)
all_ecfp <- unique(ecfp_mappings$HGNC_ID)
all_ecfp <- all_ecfp[!is.na(all_ecfp)]
write.csv(xfp_mappings, "data/target_prediction/ppb2/ecfp_mappings.csv", row.names = FALSE)
write.csv(all_ecfp, "data/target_prediction/ppb2/ecfp_targets.csv", row.names = FALSE)
...
all_xfp <- read_csv("data/target_prediction/ppb2/xfp_targets.csv", show_col_types = FALSE)$x
all_chembl <- read_csv("data/target_prediction/chemblpred/chemblpred_targets.csv", show_col_types = FALSE)$x
all_ecfp <- read_csv("data/target_prediction/ppb2/ecfp_targets.csv", show_col_types = FALSE)$x

# Coloque todos os vetores em uma lista nomeada
sets <- list(
  chembl = all_chembl,
  stp = all_stp,
  sea = all_sea,
...
# Gerar todas as combinações de 2 até o máximo
