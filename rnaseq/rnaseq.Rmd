...
gsea_dir <- "gsea_outputs_hallmark"

contrast_files <- list.files(
  gsea_dir,
  pattern = "_fgsea_hallmark_.*\\.csv$",
  full.names = TRUE
)

# nome do contraste = prefixo antes de "_fgsea_hallmark_"
gsea_results_df <- contrast_files %>%
  setNames(nm = basename(.) %>% sub("_fgsea_hallmark_.*\\.csv$", "", .)) %>%
  lapply(function(f) {
    readr::read_csv(f, show_col_types = FALSE)
  })

# conferência rápida
lapply(gsea_results_df, class)

#========================
# 1. FUNÇÃO POR DATASET
#========================

extract_top_genes <- function(gsea_df, deg_df,
                              id_type = c("ensembl", "entrez"),
                              padj_thr = 0.25) {
  id_type <- match.arg(id_type)

  # garantir tibble/data.frame
  if (!inherits(gsea_df, c("data.frame", "tbl_df"))) {
    gsea_df <- dplyr::bind_rows(gsea_df)
  }
  ...
  if (nrow(gsea_sig) == 0) {
    return(list(top_genes = tibble(), gene_rank = tibble()))
  }

  # detectar coluna de leading edge
  le_col <- grep("leading", names(gsea_sig), ignore.case = TRUE, value = TRUE)[1]
  if (is.na(le_col)) {
    stop("Nenhuma coluna de leading edge encontrada nesse resultado de GSEA.")
  }

  # leadingEdge pode vir como list ou char; forçar char e fazer split
  gsea_sig <- gsea_sig %>%
    mutate(
      le_char   = as.character(.data[[le_col]]),
      genes_list = strsplit(le_char, ",")
    )

  le <- gsea_sig %>%
    select(pathway, NES, padj_pathway = padj, genes_list) %>%
    unnest(genes_list) %>%
    transmute(
      pathway,
      NES,
      padj_pathway,
      gene_clean = trimws(genes_list)
    ) %>%
  ...
  # preparar DEGs (harmonizar IDs)
  deg_df2 <- deg_df %>%
    mutate(
      gene = as.character(gene),
      gene_clean = if (id_type == "ensembl") {
        sub("\\.\\d+$", "", gene)
      } else {
        gene
      }
  ...
  merged <- le %>%
    left_join(
      deg_df2 %>% select(gene_clean, log2FoldChange, padj),
      by = "gene_clean"
    )
  ...
  gene_rank <- merged %>%
    ...
    summarise(
      ...
      max_abs_fc = max(abs(log2FoldChange), na.rm = TRUE),
      min_padj   = min(padj, na.rm = TRUE),
      .groups    = "drop"
    ) %>%
    arrange(desc(n_pathways), desc(mean_NES), desc(max_abs_fc))

  top_genes <- if (nrow(gene_rank) > 0) {
    slice_head(gene_rank, n = 30)
  } else {
    gene_rank
  }

  list(top_genes = top_genes, gene_rank = gene_rank)
}

#========================
# 2. INFO DOS DEGs
#========================
# Certifique-se de ter isso já no ambiente:
# deg_results <- list(
#   `101521_DLPFC_ns` = res_101521_ns$res,
#   `101521_DLPFC_s`  = res_101521_s$res,
#   `80655_AnCg`      = res_80655_by_region[["AnCg"]]$res,
#   `80655_nAcc`      = res_80655_by_region[["nAcc"]]$res,
#   `80655_DLPFC`     = res_80655_by_region[["DLPFC"]]$res,
#   `214921_OFC`      = res_214921$res
# )

# tipos de ID por contraste
id_type_by_contrast <- c(
  `101521_DLPFC_ns` = "ensembl",
  `101521_DLPFC_s`  = "ensembl",
  `80655_AnCg`      = "ensembl",
  `80655_nAcc`      = "ensembl",
  `80655_DLPFC`     = "ensembl",
  `214921_OFC`      = "entrez"  # troque para "ensembl" se esse também for ENSG
)
...
for (ds in names(gsea_results_df)) {
  cat("=== PROCESSANDO:", ds, "===\n")

  gsea_df <- gsea_results_df[[ds]]
  deg_df  <- deg_results[[ds]]
  id_type <- id_type_by_contrast[[ds]]

  out <- extract_top_genes(
    gsea_df  = gsea_df,
    deg_df   = deg_df,
    id_type  = id_type,
    padj_thr = 0.25
  ...
  all_top_genes[[ds]] <- out

  if (nrow(out$gene_rank) > 0) {
    write_csv(out$gene_rank, file.path(gsea_dir, paste0("gsea_gene_rank_", ds, ".csv")))
    write_csv(out$top_genes, file.path(gsea_dir, paste0("gsea_top_genes_", ds, ".csv")))
  }
}

# Você pode inspecionar, por exemplo:
# all_top_genes$`101521_DLPFC_ns`$top_genes
...
prepare_wgcna_matrix <- function(counts, meta, sample_col,
  ...
  # 1) Checar coluna de amostra
  if (!sample_col %in% colnames(meta)) {
    stop("A coluna '", sample_col, "' não existe na tabela meta.")
  }

  # 2) Harmonizar nomes de amostras (como no find_degs)
  meta[[sample_col]] <- make.names(as.character(meta[[sample_col]]))
  colnames(counts)   <- make.names(colnames(counts))

  # 3) Filtrar meta para amostras que existem no counts
  ...
  # 5) Reordenar meta para bater com colnames(expr)
  meta <- meta[match(colnames(expr), meta[[sample_col]]), , drop = FALSE]

  # 6) Selecionar covariáveis presentes
  covars_present <- intersect(covars, colnames(meta))

  if (length(covars_present) > 0) {
    # remover amostras com NA em qualquer covariável usada
...
run_wgcna <- function(expr_resid, meta, trait_col = "condition") {
  datExpr <- t(expr_resid)   # WGCNA precisa: samples x genes
  trait <- meta[[trait_col]]
  
  # converter trait p/ numérico (Control=0, MDD=1)
  if (is.factor(trait)) trait <- as.character(trait)
  trait_num <- ifelse(trait %in% c("MDD","MDD-S","MDD-NS"), 1, 0)

  # detectar outliers grosseiros
...
extract_module_genes <- function(expr_resid, moduleColors, MEs, trait_num, p_thr = 0.05) {
  ...
  moduleTraitCor <- cor(MEdf, trait_num, use="pairwise.complete.obs")
  moduleTraitP <- corPvalueStudent(moduleTraitCor, nrow(datExpr))

  sig_modules <- rownames(MEdf)[which(moduleTraitP < p_thr)]
  ...
  for (mod in sig_modules) {
    ...
    kME <- cor(datExpr, MEdf[,mod], use="p")
...
expr_resid_80655_DLPFC <- prepare_wgcna_matrix(
  counts = cnt_80655,
  meta   = pheno_80655 %>% filter(brain_region == "DLPFC"),
  "description",
  covars = c("age","sex","pmi","rin","brain_ph")
)
...
module_genes <- extract_module_genes(
  ...
  MEs = wgcna_out$MEs,
  trait_num = ifelse(pheno_80655$group == "MDD",1,0),
  p_thr = 0.05
)
