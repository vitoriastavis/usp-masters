...
# Read SEA dataframe
sea_result <- read_excel("data/sea/sea-results.xlsx")
...
targets_list <- list(toupper(all_sea), toupper(all_stp), toupper(all_ppb), toupper(all_multitask))
targets_2plus <- names(which(table(unlist(targets_list)) >= 2))
```


# Getting metabolites for STP, PPB2 and Multitask
...
get_targets_to_metabolites <- function(folder_path) {
  # Listar todos os arquivos CSV
    ...
    csv_files <- csv_files[!grepl("^stp", basename(csv_files))]
...
get_targets_to_metabolites <- function(folder_path) {
  ...
  # Listar todos os arquivos CSV
  csv_files <- list.files(path = folder_path, pattern = "\\.csv$", full.names = TRUE)
  csv_files <- csv_files[!grepl("^multitask", basename(csv_files))]
...
names(target_metabolites_multitask) <- toupper(names(target_metabolites_multitask))
...
get_targets_to_metabolites <- function(folder_path) {
  ...
  csv_files <- csv_files[!grepl("^ppb", basename(csv_files))]
...
save(target_metabolites_ppb_chemblid, file="data/ppb2/target_metabolites_ppb2_chemblid.RData")
