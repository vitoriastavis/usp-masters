...
```{r}
gutmgene_m$Genus <- sub(" .*", "", gutmgene_m$Gut_Microbe_Name)
gutmgene_m$Genus <- gsub("\\[|\\]", "", gutmgene_m$Genus)
gutmgene_m$Genus <- gsub("\\.|\\]", "", gutmgene_m$Genus)

# Count occurrences of each genus
genus_counts <- table(gutmgene_m$Genus)

# Filter to only include genera with frequency > 5
genera_to_keep <- names(genus_counts[genus_counts >= 5])
gutmgene_filtered <- gutmgene_m[gutmgene_m$Genus %in% genera_to_keep, ]
...
gutmgene_filtered_count <- gutmgene_filtered_count[
  gutmgene_filtered_count$Genus != "uncultured",
]
gutmgene_filtered_count <- gutmgene_filtered_count[
  gutmgene_filtered_count$Genus != "unclassified",
]
gutmgene_filtered_count <- gutmgene_filtered_count[
  gutmgene_filtered_count$Genus != "bacterium",
]

p_genus <- ggplot(gutmgene_filtered_count) +
  aes(x = Genus, y = n) +
  geom_col(fill = "#6FAF9C") +
  scale_y_continuous(
    breaks = seq(0, max(gutmgene_filtered_count$n), by = 10),
    limits = c(0, max(gutmgene_filtered_count$n))
  ) +
  labs(x = "Genera", y = "Number of produced metabolites") +
  theme_classic() +
  theme(
