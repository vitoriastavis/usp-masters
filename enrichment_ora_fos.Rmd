...
key_terms <- c(
  ...
  "factor VIII", "factor IX", "tissue factor", "antithrombin", "plasma", "clotting", "growth",
  "protein S", "plasmin", "fibrinolysis", "D-dimer", "heparin", "warfarin",
...
GSE144136_celltype_degs <- rownames(GSE144136_celltype_filtered$Ast)

gse_path <- "data/snrnaseq/GSE213982/"
load(paste0(gse_path, "type_degs/GSE213982_celltype.RData"))
GSE213982_celltype_degs <- rownames(GSE213982_celltype_filtered$Ast)
```


```{r message=FALSE, warning=FALSE}
related_genes_fos <- list()
all_degs <- list(GSE144136_celltype_degs, GSE213982_celltype_degs)
names(all_degs) <- c("GSE144136_celltype", "GSE213982_celltype")
...
for (dataset in names(all_degs)) {
  ...
  e_bp <- enrichment_go(gene_ids, "BP", 0.1, 0.1, 2, 1000)
  
  e_filtered <- e_bp@result %>%
                    filter(grepl("FOS", geneID))
  
  # categories <- e_bp@result$Description
  
  categories <- filter_categories(e_bp, p_value, 50)
  # print(categories)
  
  genes_categories <- e_filtered[e_filtered$Description %in% categories, c("geneID")]

  related_genes_fos[[dataset]] <- unique(unlist(strsplit(genes_categories, "/")))
}

# save(related_genes_fos, file="genes_fos.RData")
