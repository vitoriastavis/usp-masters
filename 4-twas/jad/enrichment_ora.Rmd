...
create_graph <- function(enrichment, remove_terms, n_categories){ 
  colors <- unname(Polychrome::createPalette(n_categories, "#A665B5"))
  ...
  graph <- graph +
    theme(plot.margin = margin(t = 4, r = 4, b = 4, l = 4),
           ...
           legend.text = element_text(size = 34, margin = margin(t = 20, b = 20)),
...
for (geneset in names(enrichments)){
  ...
  # } else {
    ...
    tryCatch({
        ...
        graph <- create_graph(e, remove_terms, n_categories)
    
        save_files(graph, geneset, output_path, graph_h = 18, graph_w = 30)
